...
public class PersonaControllerV1 {
	...
	public PersonaResponse actualizarPersona(@PathVariable Integer dni, @RequestBody PersonaRequest request) {
		log.info("Into actualizarPersona REST API");
		return personaInputAdapterRest.actualizarPersona(dni, request);
	}

	@ResponseBody
	@DeleteMapping(path = "/{dni}/{database}", produces = MediaType.APPLICATION_JSON_VALUE)
	public Boolean eliminarPersona(@PathVariable Integer dni, @PathVariable String database) {
