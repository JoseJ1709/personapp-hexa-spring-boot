...
public class PhoneOutputAdapterMongo implements PhoneOutputPort {
    ...
    public Phone findById(String number) {
        log.debug("Into findById on Adapter MongoDB - Number: {}", number);

        Optional<TelefonoDocument> telefonoOpt = telefonoRepositoryMongo.findById(number);

        if (telefonoOpt.isEmpty()) {
            log.warn("Phone with number {} not found in MongoDB", number);
            return null;
        }

        TelefonoDocument telefonoDocument = telefonoOpt.get();
        log.debug("Phone found: {}", telefonoDocument);

        // ← AGREGADO: Log del primaryDuenio para debug
        if (telefonoDocument.getPrimaryDuenio() == null) {
            log.warn("Phone {} has null primaryDuenio, owner will be null", number);
        }
